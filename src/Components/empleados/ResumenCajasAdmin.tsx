...
import { useParams } from "react-router-dom";  // ✅ Importamos useParams para obtener el slug

export default function ResumenCajasAdmin({ slug }: { slug: string }) {
    ...
    useEffect(() => {
        ...
        const obtenerCajas = async () => {
            const ref = collection(db, "tiendas", slug, "cajas");  // ✅ Usamos slug
            ...
            const cajasConVentas = await Promise.all(
                snap.docs.map(async (docSnap) => {
                    ...
                    const ventasSnap = await getDocs(
                        collection(db, "tiendas", slug, "cajas", docSnap.id, "ventas")  // ✅ Usamos slug
                    );
    ...
    const borrarSeleccionadas = async () => {
        ...
        for (const id of seleccionadas) {
            await deleteDoc(doc(db, "tiendas", slug, "cajas", id));  // ✅ Usamos slug
        }
    ...
    const marcarComoRetirada = async (id: string) => {
        const ref = doc(db, "tiendas", slug, "cajas", id);  // ✅ Usamos slug
    ...
    const toggleDetalle = async (id: string) => {
        if (detalleVentas[id]) {
            ...
            const ref = collection(db, "tiendas", slug, "cajas", id, "ventas");  // ✅ Usamos slug
