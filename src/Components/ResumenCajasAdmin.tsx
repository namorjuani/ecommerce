...
export default function ResumenCajasAdmin({ slug }: { slug: string }) {
    ...
    useEffect(() => {
        ...
        const obtenerCajas = async () => {
            const ref = collection(db, "tiendas", slug, "cajas");
            ...
            const cajasConVentas = await Promise.all(
                snap.docs.map(async (docSnap) => {
                    ...
                    const ventasSnap = await getDocs(
                        collection(db, "tiendas", slug, "cajas", docSnap.id, "ventas")
                    );
    ...
    const borrarSeleccionadas = async () => {
        ...
        for (const id of seleccionadas) {
            await deleteDoc(doc(db, "tiendas", slug, "cajas", id));
        }
    ...
    const marcarComoRetirada = async (id: string) => {
        const ref = doc(db, "tiendas", slug, "cajas", id);
    ...
    const toggleDetalle = async (id: string) => {
        if (detalleVentas[id]) {
            ...
            const ref = collection(db, "tiendas", slug, "cajas", id, "ventas");
                ...
                <tbody>
                    {cajas.map((caja) => (
                        <React.Fragment key={caja.id}>
                            <tr style={{ backgroundColor: caja.retirado ? "#e0e0e0" : "#fff" }}>
                                <td style={td}>
                                    <input
                                        type="checkbox"
