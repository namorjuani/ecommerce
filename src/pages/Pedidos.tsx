...
interface Pedido {
    id: string;
    cliente: { nombre?: string; dni?: string; direccion?: string; telefono?: string; email?: string };
    productos: { id: string; nombre?: string; cantidad: number; subtotal?: number }[];
    total: number;
...
export default function Pedidos() {
    ...
    const prevCantidadRef = useRef<number>(0);
    ...
    useEffect(() => {
        ...
        const cargarPedidos = async () => {
            ...
            if (prevCantidadRef.current && lista.length > prevCantidadRef.current) {
                new Audio("/sonidos/nuevo-pedido.mp3").play();
                Swal.fire({
            ...
            prevCantidadRef.current = lista.length;
            setPedidos(lista);
        };

        cargarPedidos();
        const interval = setInterval(cargarPedidos, 10000); // revisa cada 10s
    ...
    const formatearFecha = (fecha: Timestamp | string): string => {
        ...
        else if ("toDate" in fecha) d = fecha.toDate();
        else return "";
        ...
        <div className="pedidos-container">
            ...
            {verHistorial && (
                <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", alignItems: "center" }}>
                    <label>Desde: <input type="date" value={fechaDesde} onChange={(e) => setFechaDesde(e.target.value)} style={{ padding: "0.3rem" }} /></label>
                    <label>Hasta: <input type="date" value={fechaHasta} onChange={(e) => setFechaHasta(e.target.value)} style={{ padding: "0.3rem" }} /></label>
                </div>
            )}

            {pedidosFiltrados.length === 0 ? <p>No hay pedidos para mostrar.</p> : pedidosFiltrados.map((pedido) => (
                <div key={pedido.id} className="pedido-card">
                    <div className="pedido-info">
                        ...
                        <p><strong>Teléfono:</strong> {pedido.cliente?.telefono || "Sin teléfono"} {pedido.cliente?.telefono && <a href={`https://wa.me/${pedido.cliente.telefono.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" style={{ marginLeft: "0.5rem" }}>📲</a>}</p>
                    ...
                    <ul className="pedido-productos">
                        {pedido.productos?.map((producto, i) => (
                            <li key={i}>{producto?.nombre || "Producto inválido"} x {producto.cantidad}</li>
                        ))}
                    </ul>

                    {!verHistorial && pedido.estado !== "realizado" && (
                        <div className="estado-buttons">
                            {pedido.estado === "pendiente" && <button onClick={() => cambiarEstado(pedido.id, "en_proceso")}>Marcar como En Proceso</button>}
